...
import { Mail, Phone, MapPin, Linkedin, Download, Server, Shield, Cloud, Network, HardDrive, Wrench, ExternalLink } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";

export default function ThomasBoppCV() {
  const experiences = [
    {
      ...
      tasks: [
        ...
        "Configuration de passerelles Fortinet et automatisation de déploiements GPO / scripts / Atera."
      ]
    },
    {
      role: "Freelance IT",
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-6 py-20 md:grid-cols-[1.1fr_0.9fr] md:px-10 lg:py-28">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="flex flex-col justify-center">
            <p className="mb-4 text-sm font-semibold uppercase tracking-[0.35em] text-cyan-300">Administrateur système & réseau</p>
            <h1 className="text-5xl font-bold tracking-tight text-white md:text-7xl">Thomas Bopp</h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
              Professionnel IT basé en Suisse romande, spécialisé dans l’administration système, les infrastructures réseau, la virtualisation, la sauvegarde et le support technique.
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
              <Button className="rounded-2xl px-6 py-6 text-base">
                <Download className="mr-2 h-5 w-5" /> Télécharger le CV
              </Button>
              <Button variant="outline" className="rounded-2xl border-slate-600 bg-white/5 px-6 py-6 text-base text-white hover:bg-white/10">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.1 }}>
            <Card className="rounded-[2rem] border-white/10 bg-white/10 shadow-2xl backdrop-blur">
              <CardContent className="p-8">
                <div className="mx-auto flex h-40 w-40 items-center justify-center rounded-full border-4 border-cyan-300/50 bg-slate-800 text-5xl font-bold text-white shadow-xl">TB</div>
                <div className="mt-8 space-y-4 text-slate-200">
                  ...
                  <div className="flex items-center gap-3"><Linkedin className="h-5 w-5 text-cyan-300" /> linkedin.com/in/thomas-bopp-184a81295</div>
                </div>
              </CardContent>
            </Card>
          </motion.div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10">
        <div className="grid gap-8 md:grid-cols-3">
          <Card className="rounded-3xl border-white/10 bg-white/[0.04] md:col-span-2">
            <CardContent className="p-8">
              <h2 className="text-3xl font-bold text-white">À propos</h2>
              <p className="mt-4 text-lg leading-8 text-slate-300">
                Actuellement en poste chez CDS Engineering à Villeneuve, je suis à la recherche de nouvelles opportunités dans le domaine des systèmes et réseaux afin de relever de nouveaux défis et enrichir mon expertise.
              </p>
            </CardContent>
          </Card>
              ...
              <Card className="rounded-3xl border-white/10 bg-white/[0.04]">
                <CardContent className="p-8">
                  <div className="flex flex-col justify-between gap-3 md:flex-row md:items-start">
                    ...
                    <span className="rounded-full bg-cyan-300/10 px-4 py-2 text-sm font-medium text-cyan-200">{exp.period}</span>
                  </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {skills.map((skill) => {
            ...
            return (
              <Card key={skill.title} className="rounded-3xl border-white/10 bg-white/[0.04] transition hover:bg-white/[0.07]">
                <CardContent className="p-7">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-cyan-300/10 text-cyan-300"><Icon className="h-6 w-6" /></div>
        ...
        <Card className="rounded-[2rem] border-cyan-300/20 bg-gradient-to-br from-cyan-300/10 to-blue-500/10">
          <CardContent className="grid gap-8 p-8 md:grid-cols-[1fr_auto] md:items-center md:p-10">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-slate-300">Disponible pour discuter d’opportunités en administration système, réseau, infrastructure, support IT ou projets de modernisation.</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Button className="rounded-2xl px-5 py-6"><Mail className="mr-2 h-5 w-5" /> Me contacter</Button>
              <Button variant="outline" className="rounded-2xl border-slate-600 bg-white/5 px-5 py-6 text-white hover:bg-white/10"><ExternalLink className="mr-2 h-5 w-5" /> LinkedIn</Button>
            </div>
          </CardContent>
        </Card>
      </section>
