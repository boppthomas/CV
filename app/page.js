...
const experiences = [
  {
    ...
    impact:
      "Gestion d’environnements IT multi-clients, administration système, réseau, virtualisation et sécurité.",
    tasks: [
  ...
  },
  {
    ...
    period: "2018 – 2023",
    impact:
      "Accompagnement de clients de l’analyse du besoin jusqu’à la mise en production.",
    tasks: [
...
const skills = [
  {
    icon: Server,
    title: "Systèmes",
    text: "Windows, Linux, serveurs, GPO, administration quotidienne.",
  },
  {
    icon: Network,
    title: "Réseaux",
    text: "Infrastructure, accès distant, Fortinet, sécurité réseau.",
  },
  {
    icon: HardDrive,
    title: "Virtualisation",
    text: "VMware, Hyper-V, Proxmox, sauvegardes et restauration.",
  },
  {
    icon: Cloud,
    title: "Cloud",
    text: "Solutions cloud, migration, modernisation et maintenance.",
  },
  {
    icon: Shield,
    title: "Sécurité",
    text: "Firewall, travail à distance sécurisé, protection des données.",
  },
  {
    icon: Wrench,
    title: "Support",
    text: "Dépannage, incidents, utilisateurs, documentation et formation.",
  },
];
...
function Card({ children, className = "" }) {
  return (
    <div
      className={`rounded-[2rem] border border-white/10 bg-white/[0.045] shadow-2xl backdrop-blur ${className}`}
    >
      {children}
    </div>
  );
...
function SectionTitle({ eyebrow, title, text }) {
  return (
    <div className="mb-10 max-w-3xl">
      <p className="text-xs font-bold uppercase tracking-[0.35em] text-squidGreen">
        {eyebrow}
      </p>
...
export default function Home() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-ink text-slate-100">
      ...
      <section
        id="top"
        className="relative z-10 mx-auto grid max-w-7xl gap-12 px-6 pb-20 pt-12 md:grid-cols-[1.05fr_.95fr] md:px-10 md:pb-28 md:pt-20"
      >
        <motion.div
        ...
        >
          ...
          <h1 className="max-w-4xl text-4xl font-black tracking-tight text-white md:text-6xl">
            ...
            <span className="mt-3 block bg-gradient-to-r from-squidPink to-squidGreen bg-clip-text text-transparent">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
            ...
            spécialisé dans les systèmes, réseaux, virtualisation,
            sauvegarde, sécurité et accompagnement utilisateurs.
          </p>
        ...
        >
          ...
          <Card className="mt-5 p-7">
            <p className="text-sm font-bold uppercase tracking-[0.25em] text-squidPink">
              À propos
            </p>

            <p className="mt-4 leading-7 text-slate-300">
              Actuellement en poste chez CDS Engineering à Villeneuve,
              je recherche de nouvelles opportunités dans le domaine
              des systèmes et réseaux afin de relever de nouveaux défis.
            </p>
          </Card>
        </motion.div>
      </section>
      ...
      >
        <SectionTitle
          eyebrow="Expertise"
          title="Un profil IT opérationnel et polyvalent"
          text="Une approche orientée terrain : comprendre le besoin, sécuriser l’existant, déployer proprement et accompagner les utilisateurs."
        />
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {skills.map((skill) => {
            ...
            return (
              <Card
                key={skill.title}
                className="p-7 transition hover:-translate-y-1 hover:border-squidPink/40"
              >
                <div className="flex h-13 w-13 items-center justify-center rounded-2xl bg-squidPink/10 text-squidPink">
              ...
              <Card className="p-7 md:p-9">
                ...
                <div className="mt-7 grid gap-3 md:grid-cols-2">
                  {exp.tasks.map((task) => (
                    <div
                      key={task}
                      className="flex gap-3 rounded-2xl bg-white/[0.03] p-4 leading-7 text-slate-300"
                    >
        ...
        <Card className="p-8">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            {strengths.map((item) => (
              <span
                key={item}
                className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-slate-200"
              >
                {item}
              </span>
            ))}
          ...
          <p className="mt-5 leading-7 text-slate-300">
            Un profil structuré, orienté solution et capable de faire
            le lien entre technique et utilisateurs.
          </p>
        </Card>
      </section>
        ...
        <Card className="overflow-hidden border-squidPink/20">
          <div className="grid gap-8 p-8 md:grid-cols-[1fr_auto] md:items-center md:p-10">
            <div>
              ...
              <h2 className="mt-4 text-4xl font-black text-white">
                Discutons infrastructure, systèmes ou réseau.
              </h2>

              <p className="mt-4 max-w-2xl leading-8 text-slate-300">
                Disponible pour échanger autour d’opportunités IT,
                de projets de modernisation, de support ou
                d’administration d’infrastructures.
              </p>
            </div>
      ...
      <footer className="relative z-10 mx-auto flex max-w-7xl flex-col gap-3 border-t border-white/10 px-6 py-8 text-sm text-slate-500 md:flex-row md:items-center md:justify-between md:px-10">
        <p>
          © {new Date().getFullYear()} Thomas Bopp — CV digital
        </p>
